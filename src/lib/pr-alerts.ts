...
function classifyCategory(text: string): SocialCategory {
  if (/跑單|接單|靠行|當司機|做司機|車行|跑車|接單率|加盟司機/i.test(text)) return 'driver'
  if (/機場接送|送機|接機|去機場|airport/i.test(text)) return 'shuttle'
  if (/機車|wemo|goshare|共享機車|電動機車/i.test(text)) return 'scooter'
  if (/計程車|taxi|叫車|司機|uber|yoxi|55688/i.test(text)) return 'taxi'
  return 'rental'
}
...
export function detectAlerts(posts: SocialPost[], opts: DetectAlertsOptions = {}): PrAlert[] {
  ...
  if (prior.length >= clusterMin && recent.length >= prior.length * spikeMultiplier) {
    ...
    alerts.push({
      ...
      category: latest ? classifyCategory(`${latest.title ?? ''} ${latest.description ?? ''} ${latest.keyword ?? ''}`) : 'rental',
